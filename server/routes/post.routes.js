...
const { createPost, fetchPosts, fetchUserPost, updateUserPost, updatePostReaction, deleteUserPost, deleteAnyPost } = require('../controllers/postController.js');
...
routes.post('/create-post', ensureAuthenticated, restrictTo(['admin', 'user']), checkSchema(createPostSchema), postDataValidation, createPost)
...
routes.put('/my-posts/:_id', ensureAuthenticated, restrictTo(['admin', 'user']), checkSchema(updatePostSchema), postDataValidation, updateUserPost)

routes.patch('/:id', ensureAuthenticated, restrictTo(['admin', 'user']), updatePostReaction)

routes.delete('/my-posts/:id', ensureAuthenticated, restrictTo(['admin', 'user']), checkSchema(deletePostSchema), postDataValidation, deleteUserPost)
