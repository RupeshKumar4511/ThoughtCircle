        ...
        <div className='border-white rounded px-2 py-2 my-3 bg-white shadow-md w-full' >
            ...
            <div className="py-2">
                ...
                <div className="flex gap-4 px-1 mt-2">
                    <button className="flex justify-center items-center px-2 gap-1"><AiOutlineLike /> <span className="text-pink-500" onClick={()=>postReaction({id:post.id,reaction:{like:true,dislike:false}})}>{post.like}</span></button>
                    <button className="flex justify-center items-center px-2 gap-1"><AiOutlineDislike className="relative top-0.5"/><span className="text-yellow-500" onClick={()=>postReaction({id:post.id,reaction:{dislike:true,like:false}})}>{post.dislike}</span></button>
                </div>
            </div>
        </div>
    )
